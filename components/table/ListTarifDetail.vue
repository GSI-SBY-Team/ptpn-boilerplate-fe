  ...
  <v-card>
    ...
    <v-card-text>
      <v-row>
        ...
        <v-col cols="12" md="2">
          <v-autocomplete
            ...
            v-model="filter.jenisKendaraanId"
      ...
      >
        ...
        <template v-slot:[`item.asalNama`]="{ item }">
          <span>{{ item.asalNama }} - </span>
          <span class="asal-type">{{ item.asalType }}</span>
        </template>

        <template v-slot:[`item.tujuanNama`]="{ item }">
          <span>{{ item.tujuanNama }} - </span>
          <span class="asal-type">{{ item.tujuanType }}</span>
        </template>
  ...
  methods: {
    ...
    handleApplyFilter() {
      ...
      this.filter = {
        ...
        komoditasId: filter.komoditasId,
        jenisKendaraanId: filter.jenisKendaraanId,
        t: Date.now(),
      };
      this.$router.replace({
