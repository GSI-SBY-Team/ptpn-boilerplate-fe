  ...
  <v-card>
    ...
    <v-card-text>
        ...
        <v-col md="12">
          <v-btn-toggle divided variant="outlined">
            <v-btn @click="handleExport()">
              ...
              <span class="hidden-sm-and-down">Export Excel</span>
            </v-btn>
            <v-btn @click="handleExportPdf()">
              <v-icon start color="#d32f2f"> mdi-file-pdf-box </v-icon>
              <span class="hidden-sm-and-down">Export PDF</span>
            </v-btn>
          </v-btn-toggle>
        </v-col>
      </v-row>
      ...
      >
        ...
        <template v-slot:[`item.tanggalPengolahan`]="{ item }">
          {{ dateTanggal1(item.tanggalPengolahan) }}
        </template>
        ...
        <template v-slot:[`item.tanggalJemur`]="{ item }">
          {{ dateTanggal1(item.tanggalJemur) }}
        </template>
        <template v-slot:[`item.tanggalAmbil`]="{ item }">
          {{ dateTanggal1(item.tanggalAmbil) }}
        </template>
        ...
        <template v-slot:[`item.jam`]="{ item }">
          {{ minimal_time_format_no_timezone(item.jam) }}
        </template>
        <template v-slot:[`item.jamOperasi`]="{ item }">
          {{ minimal_time_format_no_timezone(item.jamOperasi) }}
        </template>
        <template v-slot:[`item.diperiksaJam`]="{ item }">
          {{ minimal_time_format_no_timezone(item.diperiksaJam) }}
        </template>
