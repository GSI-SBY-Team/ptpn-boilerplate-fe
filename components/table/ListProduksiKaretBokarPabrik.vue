  ...
  <v-card>
    ...
    <v-card-text>
      ...
      >
        ...
        <template v-slot:[`item.namaPemanens`]="{ item }">
          {{ item.kodePemasok }} - {{ item.namaPemasok }}
        </template>
        <template v-slot:[`item.harga`]="{ item }">
          Rp.{{ formatHarga(item.harga) }}
        </template>
         <template v-slot:[`item.totalKuantum`]="{ item }">
          {{ item.kuantum * item.k3lumps /100 }}
        </template>
        <template v-slot:[`item.totalHarga`]="{ item }">
          Rp.{{ formatHarga(item.harga * (item.kuantum * item.k3lumps /100)) }}
        </template>
