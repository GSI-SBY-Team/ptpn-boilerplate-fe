    ...
    <v-card-text>
      ...
      <v-row align="center">
        ...
        <v-col
          cols="12"
          md="6"
          class="text-md-right text-sm-left font-weight-medium text-h6"
        >
          Total Kg =
          {{
            tableData.items
              .reduce((sum, item) => sum + item.berat, 0)
              .toLocaleString()
          }}
        </v-col>
      </v-row>
      ...
      >
        ...
        <template v-slot:[`item.berat`]="{ item }">
          {{ item.berat.toLocaleString() }}
        </template>

        <template v-slot:[`item.status`]="{ item }">
          {{ item.alokasiArmadaId ? "Sudah Alokasi" : "Belum Alokasi" }}
        </template>
  ...
  methods: {
    ...
    handleExport() {
      this.$emit("handleExport", {
        ...
        tanggalSelesai: this.filter.tanggalSelesai,
        komoditasId: this.filter.komoditasId,
