  ...
  <v-card>
    ...
    <v-card-text>
      <v-row>
        ...
        <v-col cols="12" md="5">
          <v-autocomplete
            label="Petani/Pemasok"
            v-model="filter.petaniId"
            :items="listPetani"
            item-value="id"
            item-title="nama"
            color="primary"
      ...
      >
        ...
        <template v-slot:[`item.tanggalm`]="{ item }">
          {{ minimal_date_format_no_timezone(item.tanggalMasuk) }}
        </template>
        <template v-slot:[`item.tanggalk`]="{ item }">
          {{ minimal_date_format_no_timezone(item.tanggalKeluar) }}
        </template>
        <template v-slot:[`item.tanggall`]="{ item }">
          {{ minimal_date_format_only(item.tanggal) }}
        </template>
        <template v-slot:[`item.tanggali`]="{ item }">
          {{ dateTanggal(item.tanggalInput) }}
        </template>
        <template v-slot:[`item.tanggalKir`]="{ item }">
          {{ dateTanggal(item.tanggalKirim) }}
        </template>
        <template v-slot:[`item.tanggalPer`]="{ item }">
          {{ dateTanggal(item.tanggalPeriksa) }}
        </template>
        <template v-slot:[`item.jamDatang`]="{ item }">
          {{ minimal_date_format(item.jamDatang) }}
        </template>
        <template v-slot:[`item.jamPembalikan`]="{ item }">
          {{ minimal_time_format_no_timezone(item.jamPembalikan) }}
        </template>
        <template v-slot:[`item.jamTurun`]="{ item }">
          {{ minimal_time_format_no_timezone(item.jamTurun) }}
        </template>
        <template v-slot:[`item.startGiling`]="{ item }">
          {{ minimal_time_format_no_timezone(item.startGiling) }}
        </template>
        <template v-slot:[`item.stopGiling`]="{ item }">
          {{ minimal_time_format_no_timezone(item.stopGiling) }}
        </template>

        <template v-slot:[`item.jamIsi`]="{ item }">
          {{ minimal_time_format_no_timezone(item.jamIsi) }}
        </template>

        <template v-slot:[`item.tanggalGiling`]="{ item }">
          {{ minimal_date_format(item.tanggalGiling) }}
        </template>
        <template v-slot:[`item.tanggalFermentasi`]="{ item }">
          {{ minimal_date_format(item.tanggalFermentasi) }}
        </template>
        <template v-slot:[`item.tanggalCuci`]="{ item }">
          {{ minimal_date_format(item.tanggalCuci) }}
        </template>
        <template v-slot:[`item.tglPetik`]="{ item }">
          {{ minimal_date_format(item.tglPetik) }}
        </template>
        <template v-slot:[`item.tanggalMulai`]="{ item }">
          {{ minimal_date_format(item.tanggalMulai) }}
        </template>
        <template v-slot:[`item.tanggalSelesai`]="{ item }">
          {{ minimal_date_format(item.tanggalSelesai) }}
        </template>
...
export default {
  ...
  watch: {
    ...
    // Tambahan watcher untuk auto reset petani ketika regional berubah
    "filter.regionalId"(newVal, oldVal) {
      // Reset petaniId ketika regionalId berubah
      if (oldVal !== null && newVal !== oldVal) {
        this.filter.petaniId = null;
      }
    },
  },
};
