  ...
  <v-card>
    ...
    <v-card-text>
      <v-row>
        <v-col cols="12" md="2">
          <v-text-field
            ...
            v-model="filter.tanggalMulaiBerlaku"
            density="compact"
        ...
        <v-col cols="12" md="2">
          <v-text-field
            ...
            v-model="filter.tanggalBerakhir"
            density="compact"
      ...
      >
        ...
        <template v-slot:[`item.tanggalMulaiBerlaku`]="{ item }">
          {{ dateTanggal(item.tanggalMulaiBerlaku) }}
        </template>

        <template v-slot:[`item.tanggalBerakhir`]="{ item }">
          <div
            :class="[
              logicDeadline(item.tanggalBerakhir) <= 0
                ? 'expired-warning'
                : logicDeadline(item.tanggalBerakhir) <= 30 &&
                  logicDeadline(item.tanggalBerakhir) >= 0
                ? 'warning'
                : '',
            ]"
          >
            {{ dateTanggal(item.tanggalBerakhir) }}
          </div>
        </template>

        <template v-slot:[`item.nominal`]="{ item }">
          Rp.{{ formatHarga(item.nominal) }}
        </template>
        ...
        <template v-slot:[`item.pathFile`]="{ item }">
          <v-btn
            ...
            size="32"
            class="mr-1"
            @click="openFile(item.pathFile)"
          >
            <FileIcon class="text-primary" size="18" />
            <v-tooltip activator="parent" location="bottom"
              >Lihat File</v-tooltip
            >
          </v-btn>
        </template>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          ...
          >
            ...
            <v-tooltip activator="parent" location="bottom">
              Detail Tarif Angkutan</v-tooltip
            >
          </v-btn>
          <v-btn
...
export default {
  ...
  data() {
    return {
      ...
      sortDesc: [true],
      filter: {
        tanggalMulaiBerlaku: new Date().toISOString().substr(0, 10),
        tanggalBerakhir: new Date().toISOString().substr(0, 10),
        q: "",
  ...
  methods: {
    ...
    dateTanggal(date) {
      // Format sesuai kebutuhan, misalnya:
    ...
    openFile(url) {
      window.open(config.public.filePathTransport + url, "_blank");
    },
    ...
    handleApplyFilter() {
      ...
      this.filter = {
        ...
        tanggalMulaiBerlaku: filter.tanggalMulaiBerlaku,
        tanggalBerakhir: filter.tanggalBerakhir,
    ...
    handleExport() {
      this.$emit("handleExport", {
        tanggalMulai: this.filter.tanggal,
        tanggalBerakhir: this.filter.tanggalBerakhir,
