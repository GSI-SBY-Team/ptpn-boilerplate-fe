    ...
    <v-card-text>
      ...
      >
        ...
        <template v-slot:[`item.startDate`]="{ item }">
          {{ minimal_date_format_no_timezone(item.startDate) || "-" }}
        </template>
        <template v-slot:[`item.endDate`]="{ item }">
          {{ minimal_date_format_no_timezone(item.endDate) }}
        </template>
        <template v-slot:[`item.tanggalRencana`]="{ item }">
          {{ dateTanggal(item.tanggalRencana) }}
        </template>
      </v-data-table>
  ...
  data() {
    return {
      ...
      filter: {
        ...
        tanggalSelesai: new Date().toISOString().substr(0, 10),
        q: "",
  ...
  methods: {
    ...
    handleApplyFilter() {
      ...
      this.filter = {
        ...
        armadaId: filter.armadaId,
        driverId: filter.driverId,
        asalId: filter.asalId,
        tujuanId: filter.tujuanId,
    ...
    handleExport() {
      this.$emit("handleExport", {
        tanggalMulai: this.filter.tanggalMulai,
        tanggalBerakhir: this.filter.tanggalSelesai,
        driverId: this.filter.driverId,
        armadaId: this.filter.armadaId,
        asalId: this.filter.asalId,
        tujuanId: this.filter.tujuanId,
      });
    },
    handleAddItem() {
      this.$emit("addItem");
    ...
    handleDetailItemRuteTransit(item) {
      let a = JSON.parse(JSON.stringify(item));
      this.$emit("detailItemRuteTransit", a);
    },
    handleDetailItemAlokasiArmada(item) {
      let a = JSON.parse(JSON.stringify(item));
      this.$emit("detailItemAlokasiArmada", a);
    },
    deleteItem(item) {
